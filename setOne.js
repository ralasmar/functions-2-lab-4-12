...
// CODE HERE
const enterParameters = (parameter1,parameter2) => (`The first parameter is ${parameter1}. The second parameter is ${parameter2}`)

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(enterParameters(`cat`,`dog`))
...
Then, outside of the greeting function, invoke the greeting function, passing in the value firstName and lastName of your choice, and a callback function that takes a parameter of fullName. 
The callback function will print 'Hello, my full name is [fullName]'. 
*/

// CODE 'GREETING FUNCTION' HERE
const printName = (firstName, lastName) => console.log(`Hello my name is ${firstName} ${lastName}`)
const greeting = (firstName, lastName, cb) => {
    console.log(printName(firstName, lastName))
}
    
// INVOKE 'GREETING FUNCTION' HERE
greeting(`rola`, `alasmar`, printName)
...
/* 
Write a function called 'pricesPlusTax' that takes 2 params: an array ('prices' array) and a callback function. Inside the function, loop over the 'prices' array. For every iteration, calculate a new 
total cost (original price plus 20% tax), then push it to the 'totalCost' array. After the loop, invoke the callback function, passing in the 'totalCost' array.
*/

// CODE HERE
const pricesPlusTax = (array, cb) => {
    for (let i = 0; i < array.length; i++){
        let cost = array[i] + (array[i] * .20)
        totalCost.push(cost)
    }
    cb(totalCost)
}
...
return (num2) => {                   //return inner function 
    if (num >= 5){
